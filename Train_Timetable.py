...
def LoadFile():
    ...
    assert os.path.isfile(timetable) #gives permission for the code to use the csv file (had problems on my pc accessing it)
...
def ArrivalTimes():
    ...
    while checker == 0:   #while loop is designed so that the user can only input the options avaliable. If they do not, the question is promted again until they pick a suitable answer.
        ...
        while checker2 == 0:
            ...
            if choice == '1':
               print( 'Date                  Train 1 Train 2 Train 3 Train 4 Train 5')
               for i in range(1, 366):
                   print( timetable[i][0]+ ':     ' , timetable[i][1], ' ' , timetable[i][2], '   ' , timetable[i][3], '  ' , timetable[i][4], ' ' , timetable[i][5])
                    checker2 = 1
                    checker = 1
            elif choice =='2':
                checker1 = 0
                while checker1 == 0:
                    train = int(input("please input the train you would like to search up times for: ")) #asks the user for which train number they are looking for
...
def ManageTimes():   #Function which allows the user to search for a specific train or look
    ...
    while checker == 0:
        ...
        if choice == '1':
            ...
            checker4 = 0
            while checker4 == 0:
                choice2 = input('would you like to know all the train times in said month, or still look for a single day (y/n): ') #gives the user the option to look through the months or skip to finding the specific train using the date
                if choice2 == 'y':
                    for i in range(0, len(monthcollection)): #checks the length of the array, then prints out the date, followed by the trains of said date, then moves to the next date, until the entire month is printed
                    ...
                    checker4 = 1
                        ...
                        print("YOU HAVE ENTERED AN INCORRECT VALUE, PLEASE TRY AGAIN ")  #prompts the user to retype due to inputting an invalid option
        elif choice == '2': 
            checker = 1
            dateToEdit, position = TrainLocatorM(timetable, False, False) #collects the exact train the user is looking for, and position said train is in the array. This is useful as it will locate the line the day appears on the csv file
            checker5 = 0
            while checker5 == 0:  #loop to prevent mistakes
                ...
                if trainUser > 5 or trainUser < 1:
                     print(" SORRY, THAT TRAIN DOES NOT EXIST ON OUR TIMETABLE, PLEASE CHOOSE A DIFFERENT DATE") #error prompted when user inputs something not in the range
                else:
                    trainHours = dateToEdit[trainUser][0] + dateToEdit[trainUser][1]   #takes the hours in the train the user has picked
                    trainMinutes = dateToEdit[trainUser][3] + dateToEdit[trainUser][4]  #takes the minutes in the train the user has picked
                    checker6 = 0
                    trainUser = str(trainUser) #converts the users input into a string so that it can be used in the following print line
                    while checker6 == 0:
                        choice3 = input('Train '+  trainUser + ' is in operation at ' + trainHours + ':'+ trainMinutes + ' today.\n Would you like to change this Train? (y/n) : ') #confirmation that they picked the correct time
                        if choice3 == 'n': #if they didnt, go back and retype
                            ...
                            checker7 = 0
                            while checker7 == 0:
                                newHour = int(input('please select the new hour(24 hour format): ')) #asks the user to type in the new hour
                                if newHour > 23 or newHour < 0:
                                    print('YOU HAVE INPUTTED AN INVALID TIME PERIOD, PLEASE SELECT AGAIN') #prompt is its out of the range
                                else:
                                    newMinute = int(input('please enter the new minutes: ')) #asks user to type  in the new minutes
                                    if newMinute > 59 or newMinute < 0:
                                        print('YOU HAVE INPUTTED AN INVALID TIME PERIOD, PLEASE SELECT AGAIN') #prompt is its out of the range
                                    else:
                                        checker7 = 1
                                        newHour = str(newHour) #convers the hour the user inputted, and checks the length so that if they only inputted 1 digit, an 0 is added in front (for the spreadsheet formatting)
                                        if len(newHour ) == 1:
                                            newHour = newHour.zfill(2)
                                        newMinute = str(newMinute)
                                        print('Ok, changing the time from',  trainHours+':'+trainMinutes,'to',newHour+':'+newMinute)
                                        ...
                                        doc.loc[position , trainHeading] = newHour+':'+newMinute #replaces the item in the specific cell with what the user inputted
...
def TrainLocatorM(timetable, onlyMonth, monthNeeded):
    checker3 = 0
    while checker3 == 0:
        month = int(input("what is the month you would like to look at (1 - 12): " )) #Asks the user to input a value between 1 and 12 corresponding to the month
        if month > 12 or month < 1:
            print(" SORRY, THAT TRAIN DOES NOT EXIST ON OUR TIMETABLE, PLEASE CHOOSE A DIFFERENT MONTH") #month range, will promt the user to retype
        else:
                checker3 = 1
                month = str(month) #converts the int 'month' to a string, this allows us to add a zero if the userinput only enters a single digit. If it were a single digit, it wont match the data in the excel file, which has no single digits on the dates. (i.e 9th of march is written as 09/03 not 9/3 on the excel file)
                if len(month) == 1:
                            month = month.zfill(2) #adds a zero infront of the digit if the string is only 1 character
                monthcollection = []  #stores the months
                for k in range(1, 366):   #this loop checks the dates to see if they are the same as the date inputted by the user. If the first number of the month the user inputted = the first number of the excel file month, it will check the second. If that also matches, it will add it to the month list and move onto the next date
...
def TrainLocatorD(timetable, monthcollection, monthPosition):
    ...
    while choice4 == 0:
        ...
        day = int(input('please enter in the correct date for the train you would like to look for (1 - '+length+'): ')) #promts the user to input the date they are looking for
        if day > len(monthcollection) or day < 1:
            print(" SORRY, THAT TRAIN DOES NOT EXIST ON OUR TIMETABLE, PLEASE CHOOSE A DIFFERENT DATE") #if number is out of range, print and prompt them to retype
        else:
            choice4 = 1
            day = str(day)   #same as months, changed int into string to allow the addition of a zero of the string has a length of 1
            if len(day) == 1:
                day = day.zfill(2)
            trainSpecific = ''
            for l in range (0, len(monthcollection)):  #again, checks the dates against the dates in the arrays, and if they are the same in both regards, they are added to the variable 'trainSpecific']
...
def AnalysisGraph():
    ...
    for i in range(1, len(timetable)):
        ...
        trains.append(timetable[i][train]) #adds the train time (which the user specified for) for the entire year into spreadsheet
        hour = int(timetable[i][train][0] + timetable[i][train][1])   #the time in the spreadsheet is converted into seconds and stored in their respected y arrays
    ...
    decider = (difference // 900) + 1  #divides range by 900 seconds (15 mins)  -> the intervals of the graph is supposed to be every 15 mins, but whilst testing, the dates seemed to be joined up and difficult to read
    ...
    wholeMin = (mini // multiplier) #divides the lowest amount of seconds by the divisions, the process behind this is to get the lowest division and highest divisions by finding what multiple is closest to the lowest and highest time found in the month
    ...
    for i in range(wholeMin, wholeMax + 1):  #within the range of the amount of divisions between the highest and the lowest times, it will convert each multiple of 900 or 1800 between the highest and lowest number from seconds to hours and minutes, and then will store to be used as tick labels
        summed = multiplier*i
        newHrs = (summed) // 3600
        ...
        y.append(multiplier * i) #y will be used as the actual ticks, so we would need to know the actual divisions in seconds
    train = str(train)
    ax.plot(date, trainTimes, color = 'b', linestyle = '-' ,label = 'Train ' + train)   #plots the arrays onto the graphs, and assigns them properties
    ...
    ax.set_yticks(y) #sets the intervals on the y axis to be the amount of intervals between the highest and lowest time in seconds
    ax.set_yticklabels(multiples_between) #labels the y axis intervals in the hr:min format
    ax.set_xticklabels(dateCollection) #labels the dates to the correct dates (before adding this, the dates were set to the 02 of each month rather than the first)
...
def Schedule():   
    ...
    while checker == 0:
        print("Would you like to:\n1 - Run schedule until a certain train departs\n2 - Run Schedule for a fixed time\n3 - Run schedule forever") #gives user the choice of what is outputted on the live timetable and for how long
        choice = input('Please choose and option: ')
        if choice == '1': 
            checker1 = 0 #again, another loop maker to allow users to reattempt
            while checker1 == 0:
                train = int(input("please input the train you would like to be updated on: "))  #prompts the user to input the train they would like to use
                if train > 5 or train < 0:
                    print("THIS TRAIN IS NOT REGISTERED ON THE SYSTEM, PLEASE INPUT THE CORRECT TRAIN")  #prompts user to reinput due to an incorrect value
                else:
                    trainSchedule(False, train, 0) #sends of the train the user would like to be updated on into the next function (designed to output the live broadcast of when the train is coming), sends to the program that there are not multiple trains, the specific train they would like to know about is what they inputted, and that they want the program to run infinitely (default = 0)
                    checker1 = 1
        elif choice == '2': 
            checker2 = 0 #loop maker to allow users to reattempt
            while checker2 == 0:
                tHours = int(input('please input the amount of hours you would like the run the program for: ')) #prompts the user to input how long they would like to run the program
                if tHours < 0:
                    print('YOU HAVE INPUTTED AN INVALID TIME PERIOD, PLEASE SELECT AGAIN') #prompts user to reinput due to an incorrect value
                ...
                trainSchedule(True, 0, tSeconds) #sends the amount of seconds the user wants the program running for. Tells function that there are multiple trains, there is no specific train they want(default = 0) and that they want the program to run for tSeconds
        elif choice == '3':
            trainSchedule(True, 0, 0)  #sends to the program that there are multiple trains, no specifc train and no specific run time
        else:
            print('!!!!YOU HAVE INPUTTED AN INVALID RESPONSE, PLEASE TRY AGAIN!!!!' ) #prompts user to reinput due to an incorrect value
...
def trainSchedule(multiTrain, Train, tSeconds):
    ...
    while choice != 'x':
        ...
        if len(month) == 1:
            month = month.zfill(2) #checks the date and the month of today, and compares it to whats found on the spreadsheet. When said date is found, add it to a variable
        for i in range(1, 366):
        ...
        for i in range(1, 6):  #creates a for loop to repeat 6 times
            ...
            if  newHrs < 0:   #if the results of the difference puts the hours in minus, then the trains must have departed already
                ...
                if (newMins <= 20 and newMins%5 ==0) or (newMins < 5): #and the remaining minutes are 20, 15, 10, 5, 4, 3, 2 and 1, the user is alerted that the train is near and should be ready to board
                    print("!!!!!!! TRAIN", i,  "WILL BE DEPARTING IN", newMins, "MINUTES, PLEASE BE READY TO BOARD!!!!!!")
...
def menu():
    ...
    while checker == 0:  
        ...
        if choice == '1':
            ...
            while checker2 == 0:
                choice2 = input("are you sure you would like to quit the program (y/n) ")      #say the user doesnt want to quit, there is a secondary confirmation for it
                if choice2 == 'y':   #last confirmation
                    exit(1)
                elif choice2 == 'n':
                    ...
                    print(" YOU HAVE INPUTTED AN INVALID OPTION, ANSWER AGAIN") #program telling the user that what they inputted is not on the list and to retry it
                    time.sleep(1)
        else:
            print(" YOU HAVE INPUTTED AN INVALID OPTION, ANSWER AGAIN")     #program telling the user that what they inputted is not on the list and to retry it
            time.sleep(1)

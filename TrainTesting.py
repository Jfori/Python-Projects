#program which takes data from a time sheet and produces the following:
#1) Search for all the arrival times / look up all arrival times for a specific train
#2) Search the arrival times of trains on 1 date / change the time of a specific train
#3) Produce an arrival time graph
#4) Set up a live train schedule for today / for a set duration, which updates every minute.
...
def LoadFile():
    ...
    for row in readcontent:
        rows.append(row)
    return (rows) # readcontent[0] - heading, readcontent[1][0] - date, readcontent[1][1] - first train on said date
